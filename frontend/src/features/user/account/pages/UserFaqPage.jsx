...
export default function UserFaqPage() {
  ...
  useEffect(() => {
    const fetchFaqs = async () => {
      try {
        const res = await api.get('/web-faqs/common?category=user_app');
    ...
    <div className="flex-1 flex flex-col bg-bg min-h-dvh">
      <div className="bg-white px-4 pt-4 pb-4 shadow-sm">
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate('/user/account')} className="p-2 -ml-2 rounded-xl hover:bg-gray-100">

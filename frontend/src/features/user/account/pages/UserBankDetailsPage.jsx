...
const UserBankDetailsPage = () => {
  ...
  const cachedUser = useUserAuthStore((s) => s.user);
  const updateUser = useUserAuthStore((s) => s.updateUser);
  const profileKey = buildCacheKey('user-profile', { userId: cachedUser?._id || '' });
  const { data: profile, loading, error, refetch } = useCachedQuery(useUserProfileStore, profileKey, { userId: cachedUser?._id || '' });
  ...
  const user = profile?.user || cachedUser || {};
  ...
  if (error && !profile) {
    ...
    return (
      <div className="flex-1 flex flex-col bg-bg min-h-dvh">
        <header className="bg-white px-4 pt-5 pb-5 shadow-sm">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate('/user/account')}
              className="w-9 h-9 rounded-xl border border-border-light flex items-center justify-center text-text-secondary hover:bg-slate-50 transition-colors shrink-0"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div className="min-w-0 flex-1">
              <h1 className="text-xl font-bold text-text">Payment account</h1>
              <p className="text-sm text-text-secondary mt-0.5">Review your bank account for withdrawals.</p>
            </div>
          </div>
        </header>
        <div className="p-4 pb-8 space-y-4 flex-1">
      {!bank ? (
        <Card className="p-6 text-center space-y-4">
          <p className="text-sm font-semibold text-text">No bank details available</p>
          <p className="text-sm text-text-secondary">Your bank details will appear here once they are added to your profile.</p>
          <button
            type="button"
        ...
        <>
          ...
          <Card className="p-4 space-y-2">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              Keep your bank details accurate so withdrawals process without delays. If something changes, update your profile or contact support.
            </p>
          </Card>
        </>
      )}

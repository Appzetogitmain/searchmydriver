...
const FareCard = ({ estimate, estimating = false, error = null, dense = false, footnote = null, title = 'Fare estimate', extraTip = 0 }) => {
  ...
  const detailRows = useMemo(() => {
    ...
    if (extraTip > 0) {
      base.push(['Extra Tip (Paid directly to driver)', extraTip]);
    }

    return base
      .filter(([, value, suppress]) => !suppress && Number(value || 0) !== 0)
      .map(([label, value]) => [label, Number(value) || 0]);
  }, [breakdown, isOutstation, isMonthly, extraTip]);
  ...
  const fareTotal = (breakdown.totalPayable || 0) + (extraTip || 0);
      ...
      {!error && (
        <div className={dense ? 'space-y-1.5' : 'space-y-2.5'}>
          ...
          */}
          ...
          {bufferRupees <= 0 && (
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/80">
              <p className="font-bold text-text">Total payable</p>

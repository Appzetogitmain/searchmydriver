    ...
    <div className="flex-1 flex flex-col bg-bg min-h-dvh">
      ...
      <div className="p-4 space-y-4 flex-1">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 shadow-sm">
          ...
          {/* Stats row */}
          <div className="mt-5 flex items-center gap-4 text-[11px] text-white/60 bg-white/5 rounded-2xl p-3 border border-white/5">
            <div className="flex-1 min-w-0">
              <span className="uppercase tracking-wider text-[10px] block truncate">Added</span>
              <p className="text-[14px] font-semibold text-white mt-0.5 truncate" title={`₹${Number(wallet.totalCredited || 0).toLocaleString('en-IN')}`}>
                ₹{Intl.NumberFormat('en-IN', { notation: 'compact', maximumFractionDigits: 2 }).format(Number(wallet.totalCredited || 0))}
              </p>
            </div>
            <div className="w-px h-8 bg-white/10 shrink-0" />
            <div className="flex-1 min-w-0">
              <span className="uppercase tracking-wider text-[10px] block truncate">Spent</span>
              <p className="text-[14px] font-semibold text-white mt-0.5 truncate" title={`₹${Number(wallet.totalSpent || 0).toLocaleString('en-IN')}`}>
                ₹{Intl.NumberFormat('en-IN', { notation: 'compact', maximumFractionDigits: 2 }).format(Number(wallet.totalSpent || 0))}
              </p>
            </div>
          </div>

...
const PlatformSettings = () => {
  ...
  const fetchData = useCallback(async ({ silent = false } = {}) => {
    try {
      ...
      const [carsRes, condRes, platformSettingsRes] = await Promise.all([
      ...
      <div className="overflow-x-auto pb-1 no-scrollbar">
        <div className="flex items-center gap-1.5 p-1.5 bg-slate-100 rounded-2xl w-fit">
          {[
            ...
            { id: 'ratings', label: 'Rating Questions', icon: Star },
          ].map(tab => (
            <button
              key={tab.id}
          ...
          {activeTab === 'policies' && (
            <div className="space-y-6 max-w-2xl">
              <form onSubmit={handlePolicySubmit} className="space-y-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-800">Support Contact Details</h3>
                  <p className="text-sm text-slate-500 mt-1 mb-4">These details will be displayed publicly on the Support and Help Desk pages.</p>
                  <div className="space-y-4">
                    <Input
                      label="Support Email Address"
                      type="email"
                      value={policyForm.supportEmail}
                      onChange={(e) => setPolicyForm({ ...policyForm, supportEmail: e.target.value })}
                      required
                    />
                    <Input

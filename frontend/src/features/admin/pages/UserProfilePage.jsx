    ...
    <div className="space-y-6 animate-fade-in-up pb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-slate-600">
              ...
              {!user.email?.includes('@phone.searchmydriver.local') && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard title="Wallet details">
          {!user.wallet ? (
            <p className="text-sm text-slate-500">No wallet information available.</p>
          ) : (
            <InfoGrid
              items={[
                { label: 'Current Balance', value: `₹${user.wallet.balance || 0}` },
                { label: 'Total Credited', value: `₹${user.wallet.totalCredited || 0}` },
                { label: 'Total Spent', value: `₹${user.wallet.totalSpent || 0}` },
                { label: 'Held Amount', value: `₹${user.wallet.heldRupees || 0}` },
              ]}
            />
          )}
        </SectionCard>
              ...
              <div key={car._id} className="flex gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-slate-900">
                    {getCarBrandName(car) !== '—' ? getCarBrandName(car) : ''} {getCarModelName(car) !== '—' ? getCarModelName(car) : (car.modelName || 'Unknown Vehicle')}
                  </p>
                  ...
                  <InfoGrid
                    ...
                    items={[
                      { label: 'Category', value: getCarCategoryName(car) },
                      {
                        ...
                        value: `${getCarFuelName(car) !== '—' ? getCarFuelName(car) : 'Unknown'} · ${car.transmission || 'Unknown'}`,
                        capitalize: true,
                      },
                    ]}
                  />

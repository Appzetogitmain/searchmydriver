...
const ManageWebFaqs = ({ category = 'website', title = 'Website FAQs' }) => {
  ...
  const fetchFaqs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/web-faqs/admin?category=${category}`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          <h2 className="text-xl font-bold text-slate-900">Manage {title}</h2>
          <p className="text-sm text-slate-500 font-medium">
            Publish and manage Frequently Asked Questions shown on the {title.toLowerCase()} screen.
          </p>
        </div>
...
const FaqFormDrawer = ({ faq, category, onClose, onSaved }) => {
  const isEdit = !!faq;

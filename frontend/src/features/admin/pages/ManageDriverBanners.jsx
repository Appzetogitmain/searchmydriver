...
const ManageDriverBanners = () => {
  ...
  const fetchBanners = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/admin/banners?type=driver');
      setBanners(res?.data?.data || []);
      setError('');
    } catch (err) {
      setError(err?.response?.data?.message || 'Failed to load driver banners');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Driver banner deleted');
      setConfirmDelete(null);
      fetchBanners();
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Could not delete driver banner');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          <h2 className="text-xl font-bold text-slate-900">Manage Driver Banners</h2>
          <p className="text-sm text-slate-500 font-medium">
            Publish and manage promotional banner slides shown on the driver app homepage.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleNew} className="rounded-xl font-bold flex items-center gap-2">
            <Plus className="w-4 h-4" /> Add Driver Banner
          </Button>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center max-w-md mx-auto space-y-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium">
              Click "Add Driver Banner" to upload your first banner image.
            </p>
          </div>
        </div>
      ) : (
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        toast.success('Driver banner updated');
      } else {
        await api.post('/admin/banners', payload);
        toast.success('Driver banner created');
      }
      onSaved();
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Failed to save driver banner');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="px-5 py-4 flex items-center justify-between gap-3">
      <div className="min-w-0">
        <p className="text-[11px] uppercase tracking-wide text-slate-500">
          {isEdit ? 'Edit Driver Banner' : 'New Driver Banner'}
        </p>
        <h2 className="text-base font-bold text-slate-900 truncate">
          {isEdit ? title || 'Untitled Banner' : 'Create Driver Banner'}
        </h2>
      </div>
      <button
        type="button"
      ...
      <form onSubmit={handleSave} className="p-5 space-y-5">
        ...
        <div>
          ...
          <Input
            ...
            placeholder="e.g. Driver Dashboard Promo"
            maxLength={120}
          />
        </div>
        ...
        <div>
          ...
          <Input
            ...
            placeholder="https://yourpartner.com/driver-landing"
            icon={ExternalLink}
          />
        </div>
...
export default ManageDriverBanners;

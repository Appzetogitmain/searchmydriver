...
const buildDefaultForm = (serviceType) => ({
  ...
  outstation: {
    ...
    minDays: 1,
    maxDays: 0,
    // Legacy fallback — fare engine uses this when both foodAllowancePerDay
    // and stayAllowancePerNight are 0 on older saved pricing docs.
    allowancePerNight: 0,
  },
...
const buildFormFromExisting = (existing) => {
  ...
  return {
    ...
    oneWayCharge: { ...base.oneWayCharge, ...(existing.oneWayCharge || {}) },
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 p-5">
        <div className="space-y-6">
          ...
          <Section
            title="One-Way Trip Pricing"
            subtitle="Apply a per-km charge for one-way trips, based on the estimated distance."
          >
            <div className="p-3 bg-slate-50 rounded-xl space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-sm font-semibold block">Enable one-way charge</span>
                  <span className="text-xs text-slate-500">
                    If on, one-way trips will add an extra fee per km.
                  </span>
                </div>
                <Toggle
                  checked={form.oneWayCharge.enabled}
                  onChange={(v) => updateNested('oneWayCharge', { enabled: v })}
                />
              </div>
              {form.oneWayCharge.enabled && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Input
                    label="Rate (₹ per km)"
                    type="number"
                    min={0}
                    value={form.oneWayCharge.perKmRate}
                    onChange={(e) =>
                      updateNested('oneWayCharge', {

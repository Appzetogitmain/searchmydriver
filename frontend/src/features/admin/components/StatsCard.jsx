 ...
 */
const StatsCard = ({ icon: Icon, label, value, trend, trendLabel, color = '#FFC107', className = '', onClick }) => {
  const isPositive = trend >= 0;

  return (
    <div 
      onClick={onClick}
      className={`bg-white rounded-2xl p-5 border border-gray-100 transition-shadow ${onClick ? 'cursor-pointer hover:shadow-lg' : 'hover:shadow-md'} ${className}`}
    >

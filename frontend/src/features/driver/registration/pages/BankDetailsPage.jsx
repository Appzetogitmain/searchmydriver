...
const BankDetailsPage = () => {
  ...
  const handleChange = (f) => (e) => {
    ...
    if (f === 'account') {
      value = value.replace(/\D/g, '').slice(0, 18); // strict digits, max 18
    } else if (f === 'ifsc') {
  ...
  const handleContinue = async () => {
    if (form.account.length < 9 || form.account.length > 18) {
    ...
    if (!ifscRegex.test(form.ifsc)) {
      alert('Please enter a valid 11-character IFSC code (e.g. HDFC0001234).');
      return;
    }

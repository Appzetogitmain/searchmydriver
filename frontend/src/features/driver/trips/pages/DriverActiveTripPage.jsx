...
const NEXT_ACTION_BY_STATUS = {
  ...
  [BOOKING_STATUS.ARRIVED]: {
    ...
    cta: { label: 'Enter Start PIN', icon: PlayCircle, action: 'startTrip' },
    canCancel: true,
  },
...
const DriverActiveTripPage = () => {
  ...
  const [lastStatus, setLastStatus] = useState(booking?.status || null);

  useEffect(() => {
    if (booking?.status && !lastStatus) {
      setLastStatus(booking.status);
    }
  }, [booking?.status, lastStatus]);
  ...
  // The driver's live location for the trip map. `useDriverLocation` streams
  // GPS updates to Socket.IO + updates the local map state.
  const { coords: driverCoords } = useDriverLocation({ enabled: true });
  ...
  const routeDestination = useMemo(() => {
    if (status === BOOKING_STATUS.STARTED && dropoffCoords) {
      return dropoffCoords;
    }
    return pickupCoords;
  }, [status, dropoffCoords, pickupCoords]);
  ...
  const handleAdvance = useCallback(async () => {
    ...
    // ARRIVED → opens the PIN entry sheet instead of calling startTrip
  ...
  // Trip already completed/cancelled — render the invoice details instead of active map.
  ...
  const offlineTip = booking.fareSnapshot?.offlineTip || 0;
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg min-h-dvh">
      {/* Warning if no kit purchased */}
      {!driver?.activeKitOrderId && booking.status !== BOOKING_STATUS.STARTED && (
      ...
      <div className="flex-1 p-4 space-y-4">
        {/* Live map: driver + pickup/dropoff, emphasising the pickup pin */}
        {showMap && (
          <div className="relative">
            <TripTrackingMap
              ...
              emphasis={status === BOOKING_STATUS.STARTED ? 'driver' : 'pickup'}
              height={240}
              showRoute={booking.status !== BOOKING_STATUS.ARRIVED}
              followDriver={status === BOOKING_STATUS.STARTED}
            />
            <button
              type="button"
              onClick={() => setFullscreenMapOpen(true)}
              className="absolute top-3 right-3 z-20 inline-flex items-center justify-center rounded-full bg-white/90 shadow-lg p-2 border border-slate-200 text-text-muted hover:bg-white"
        ...
        <Card>
          ...
          {((booking.dropoff?.address && booking.dropoff.address !== booking.pickup?.address) || booking.outstation?.destinationAddress) && (
            <div className="flex items-start gap-3 mt-3 border-t border-slate-100 pt-3">
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {booking.serviceType === SERVICE_TYPES.MONTHLY ? (
                  <p className="text-base font-bold text-text">Negotiated</p>
                ) : (
          ...
          {booking.serviceType === SERVICE_TYPES.MONTHLY ? (
            <p className="text-[11px] text-text-muted mt-2 leading-snug">
              Directly negotiated and paid by the customer.
            </p>
          ) : (
            <p className="text-[11px] text-text-muted mt-2 leading-snug">
        ...
        {status === BOOKING_STATUS.STARTED && ((isCashBooking && cashDueRupees > 0) || (booking?.fareSnapshot?.offlineTip > 0)) && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[12px] text-amber-800 mt-1 leading-snug">
                  Please collect ₹{((isCashBooking ? cashDueRupees : 0) + (booking?.fareSnapshot?.offlineTip || 0)).toLocaleString('en-IN')} from the customer before tapping Complete trip.
                </p>
              </div>
            </div>
          </Card>
      ...
      {fullscreenMapOpen && (
        <div className="fixed inset-0 z-[70] bg-black/80 p-4 flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              <p className="text-sm font-semibold text-white">Live tracking</p>
              <p className="text-xs text-slate-200">Driver location and route updated in real time</p>
            </div>
            <button
              type="button"
              onClick={() => setFullscreenMapOpen(false)}
              className="inline-flex items-center gap-2 rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-slate-900 shadow-lg"
            >
              <Minimize2 className="w-4 h-4" />
              Close
            </button>
          </div>
          <div className="flex-1 rounded-[2rem] overflow-hidden bg-white shadow-2xl">
            <TripTrackingMap
              driver={driverPoint}
              pickup={pickupCoords}
              dropoff={dropoffCoords}
      ...
      <TripChatModal
        ...
        recipientPhotoUrl={customerPhoto}
        recipientModel="User"
      />
    </div>
  );
 ...
 */
function CustomerHeroCard({ photo, name, phone, email, since, onCallClick, onMessageClick }) {
  return (
      ...
      <div className="px-5 pt-3 pb-4 border-t border-border-light bg-white space-y-2">
        ...
        <div className="mt-3 flex items-center gap-3">
          {onCallClick && (
            <button
              onClick={onCallClick}
              className="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-50 text-emerald-700 font-semibold py-3 text-sm shadow-sm hover:bg-emerald-100 active:scale-[0.98] transition border border-emerald-100"
          ...
          {onMessageClick && (
            <button
              ...
              className="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-50 text-blue-700 font-semibold py-3 text-sm shadow-sm hover:bg-blue-100 active:scale-[0.98] transition border border-blue-100"
...
const DriverTripInvoiceDetails = ({ booking, onBack }) => {
  ...
  const handleDownloadPdf = async () => {
    setDownloading(true);
    try {
      const res = await api.get(`/driver/bookings/${booking._id}/invoice`, {
        responseType: 'blob',
      });
      const filename = `Invoice-${booking.invoiceNumber || booking.bookingNumber}.pdf`;
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg min-h-dvh">
      ...
      <div className="flex-1 p-4 space-y-4 pb-24">
        {/* Earnings Card */}
        <Card className="text-center py-6 border-emerald-100 bg-emerald-50/20">
          ...
          {booking.serviceType === SERVICE_TYPES.MONTHLY ? (
            ...
            <>
              ...
              <p className="text-[11px] text-text-muted mt-2">
                {isCompleted
                  ? 'Net earning credited to your wallet.'
                  : 'Trip was cancelled. Any applicable cancellation payout is shown above.'}
              </p>
            </>
          )}
        </Card>
        ...
        <Card className="space-y-3">
          <div className="flex justify-between text-xs border-b border-slate-100 pb-2">
            ...
            <span className="font-semibold text-text">{booking.invoiceNumber || '—'}</span>
          </div>
          <div className="flex justify-between text-xs border-b border-slate-100 pb-2">
            <span className="text-text-muted">Booking No:</span>
            <span className="font-semibold text-text">{booking.bookingNumber}</span>
          </div>
          <div className="flex justify-between text-xs border-b border-slate-100 pb-2">
        ...
        <Card className="space-y-4">
          <div className="flex items-start gap-3">
            <MapPin className="w-4 h-4 text-emerald-600 mt-0.5 shrink-0" />
            <div className="min-w-0">
              <p className="text-[11px] text-text-muted">Pickup Address</p>
              <p className="text-sm font-medium text-text break-words">
                {booking.pickup?.address}
              </p>
            </div>
          ...
          {((booking.dropoff?.address && booking.dropoff.address !== booking.pickup?.address) || booking.outstation?.destinationAddress) && (
            <div className="flex items-start gap-3 border-t border-slate-100 pt-3">
              <MapPin className="w-4 h-4 text-rose-500 mt-0.5 shrink-0" />
              <div className="min-w-0">
                <p className="text-[11px] text-text-muted">Dropoff Address</p>
                <p className="text-sm font-medium text-text break-words">
                  {booking.dropoff?.address || booking.outstation?.destinationAddress}
                </p>
              </div>
        ...
        {booking.carId && (
          <Card className="flex items-center gap-3">
            <CarIcon className="w-5 h-5 text-text-muted" />
            <div className="min-w-0 flex-1">
              <p className="text-[11px] text-text-muted">Vehicle driven</p>
              <p className="text-sm font-semibold text-text truncate">
                {booking.carId.brandId?.name || booking.carId.carTypeId?.name} {booking.carId.modelId?.name}
              </p>
              <p className="text-[11px] text-text-secondary">
                {booking.carId.vehicleNumber}
              </p>
            </div>
          </Card>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white border-t border-border-light px-4 py-3 z-10">
        <Button
          fullWidth
          variant="driver"
          loading={downloading}

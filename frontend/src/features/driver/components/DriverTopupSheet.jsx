...
const DriverTopupSheet = ({
  ...
  const handleSubmit = async () => {
    if (!amountValid || submitting || checkoutLoading) return;
    setSubmitting(true);
    try {
      const { data } = await api.post('/driver/wallet/topup', { amount: Number(amount) });
      const order = data?.data?.razorpay;
      ...
      await openCheckout({
        ...
        onSuccess: async (response) => {
          const verifyRes = await api.post('/driver/wallet/topup/verify', {
...
export default DriverTopupSheet;

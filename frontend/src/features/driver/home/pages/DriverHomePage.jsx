...
const DriverHomePage = () => {
  ...
  // Prime audio playback on the user gesture that toggles online so
  // subsequent programmatic `play()` calls are not blocked by autoplay
  // policies. This uses the same alert asset the app uses elsewhere.
  const { prime: primeNotification } = useNotificationSound('/audio/alert_.mp3', { volume: 0.9 });
  ...
  const handleToggle = async (next) => {
    // Known user gesture — prime audio to unlock autoplay, then toggle.
    if (next) {
      try {
        primeNotification();
      } catch {
      ...
      <div className="sticky top-0 z-50 bg-bg">
        <div className="bg-gradient-to-b from-[#FFF5D6] to-[#FFEAA8] border-b border-[#F5D169] px-4 pt-4 pb-6 rounded-b-3xl shadow-md">
          ...
          <h1 className="text-lg font-extrabold text-slate-900 capitalize">
            Hii {driver?.name?.split(' ')[0] || 'Driver'}
          </h1>

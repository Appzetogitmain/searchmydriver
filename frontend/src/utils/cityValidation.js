...
/**
 * Normalizes city name for comparison (lowercase, alphanumeric, stripping common suffixes).
 * @param {string} name
 * @returns {string}
 */
 ...
 * Typically addresses end with "... , City, State [Pincode], Country".
...
export function extractCityFromAddress(address) {
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    ...
    // Skip state + pin code (e.g., "Madhya Pradesh 452001" or "452001")
    if (
      /\d{6}/.test(part) ||
...
export function isSameCity(pickup, destination) {
  ...
  if (pickupCity && destCity) {
    ...
    // Both explicitly have different cities
    return false;
  }

...
function TripTrackingMap({
  ...
  /* ------------------------------------------------------------------ */
  const routeDestination = destination || pickup;
  const {
    path: routePath,
    ...
    destination: showRoute ? routeDestination : null,
    enabled: showRoute && Boolean(driver && routeDestination),
  });
  ...
  const fallbackEndpoint = routeDestination || pickup;
  const showFallback =
    showRoute && driver && fallbackEndpoint && (!routePath || routePath.length < 2);

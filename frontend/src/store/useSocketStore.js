...
const useSocketStore = create((set, get) => ({
  ...
  /** Track which chat modal is currently open to prevent duplicate toasts */
  ...
  _bindCoreHandlers: (socket) => {
    ...
    socket.on('NEW_CHAT_MESSAGE', (msg) => {
      const activeChatBookingId = get().activeChatBookingId;
      if (activeChatBookingId === msg.bookingId) {
        // Chat is open for this booking, let the component handle it (or ignore toast)
        return;
      }
      
      // Determine if message is from me
      const currentUserId = useUserAuthStore.getState().user?._id || useUserAuthStore.getState().user?.id || 
                            useDriverAuthStore.getState().driver?._id || useDriverAuthStore.getState().driver?.id;
      const senderId = msg.senderId?._id || msg.senderId;
      
      if (String(senderId) !== String(currentUserId)) {
        import('react-hot-toast').then(({ default: toast }) => {
          toast(`New Message: ${msg.message}`, {
            icon: '💬',
            duration: 4000,
          });
        });

...
import { getRealtimeDb, isFirebaseConfigured, getFirebaseApp } from '../config/firebase';
  ...
  useEffect(() => {
    ...
    const errorHandler = (err) => {
      if (err.message && err.message.includes('permission_denied')) {
        console.warn('Firebase permission denied. Falling back to MongoDB seed data.');
        return;
      }
        ...
        if (app) {
          ...
          if (!auth.currentUser) {
            await signInAnonymously(auth).catch(() => {
              // Ignore anonymous auth errors if not enabled
            });
          }
        }

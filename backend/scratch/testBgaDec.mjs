...
async function test(message, sid) {
  ...
  try {
    ...
    console.log(`Response for "${message}" with SID "${sid}":`, responseText);
  } catch (error) {
...
async function run() {
  // Test with BGADEC at the end as part of message
  await test('Welcome to the Search My Driver powered by Appzeto.Your OTP for registration is 999999.BGADEC', 'BGADEC');
  await test('Welcome to the Search My Driver powered by Appzeto.Your OTP for registration is 999999. BGADEC', 'BGADEC');
  await test('Welcome to the Search My Driver powered by Appzeto.Your OTP for registration is 999999.', 'BGADEC');
}

 ...
 * 12-hour booking). Outstation uses `stayAllowancePerNight` for its
 * per-night accommodation charge.
 */
...
// ─── One-Way Trip specific block ───────────────────────────────────────────
const oneWayChargeSchema = new mongoose.Schema(
  {
    enabled: { type: Boolean, default: false },
    /** ₹ per km charged for one way trips based on estimated distance */
...
const outstationSchema = new mongoose.Schema(
  {
    ...
    /**
     * @legacy Combined per-night allowance from the previous pricing revision.
     * The fare engine reads this ONLY when both `foodAllowancePerDay` and
     * `stayAllowancePerNight` are 0 (so older saved docs keep producing the
     * same fare without a manual migration). New pricing configs should use
     * the split fields above.
     */
...
const servicePricingSchema = new mongoose.Schema(
  {
    ...
    // ── One Way Trip extras ──
    oneWayCharge: { type: oneWayChargeSchema, default: () => ({}) },

...
export const adminListFaqs = asyncHandler(async (req, res) => {
  ...
  const faqs = await WebFaq.find(filter).sort({ sortOrder: 1, createdAt: -1 }).lean();
...
export const adminCreateFaq = asyncHandler(async (req, res) => {
  const { question, answer, sortOrder, isActive, category } = req.body;
...
export const listActiveFaqs = asyncHandler(async (req, res) => {
  ...
  if (req.query.category) filter.category = req.query.category;
  else filter.category = 'website'; // Default to website for backwards compatibility

...
export const listWithdrawals = async (req, res, next) => {
  try {
    ...
    const withdrawals = await WithdrawalRequest.find(query)
      ...
      .skip(skip)
      .limit(limit)
      .populate('driverId', 'name email phone_no profilePicture bankDetails')
      .populate('userId', 'name email phone_no profilePicture bankDetails')
      .lean();
...
export const approveWithdrawal = async (req, res, next) => {
  try {
    ...
    withdrawal.adminId = (req.staff || req.user)._id;
    withdrawal.processedAt = new Date();
    await withdrawal.save();

    // Notify the user/driver
    const targetUserId = withdrawal.userType === 'Driver' ? withdrawal.driverId : withdrawal.userId;
    if (targetUserId) {
      const { emitNotification } = await import('../utils/socketEmitters.js');
      const target = withdrawal.userType === 'Driver' ? { driverId: targetUserId } : { userId: targetUserId };
      emitNotification(
        target,
        {
          title: 'Withdrawal Approved',
          body: `Your withdrawal request for ₹${withdrawal.amount} has been approved and processed. Ref: ${withdrawal.transactionRef || 'N/A'}`,
          data: { withdrawalId: withdrawal._id, type: 'withdrawal_approved' },
          severity: 'success',
        }
      );
    }
...
export const rejectWithdrawal = async (req, res, next) => {
  try {
    ...
    // Refund the user/driver
    const userId = withdrawal.userType === 'Driver' ? withdrawal.driverId : withdrawal.userId;
    if (userId) {
      await creditWalletService({
        ...
        initiatedBy: (req.staff || req.user)._id,
      });

      const { emitNotification } = await import('../utils/socketEmitters.js');
      const target = withdrawal.userType === 'Driver' ? { driverId: userId } : { userId: userId };
      emitNotification(
        target,
        {
          title: 'Withdrawal Rejected',
          body: `Your withdrawal request for ₹${withdrawal.amount} was rejected. Reason: ${reason || 'Admin action'}. The amount has been refunded to your wallet.`,
          data: { withdrawalId: withdrawal._id, type: 'withdrawal_rejected' },
          severity: 'error',
        }
      );
    }

...
export const addOfflineTip = asyncHandler(async (req, res) => {
  ...
  booking.offlineTip = (booking.offlineTip || 0) + amount;
  await booking.save();
  
  return res.status(200).json(new ApiResponse(200, { booking }, 'Offline tip added'));
});
...
export const driverAcceptBooking = asyncHandler(async (req, res) => {
  if (req.driver.approvalStatus === 'suspended') {
    throw new ApiError(403, 'Your account is suspended. You cannot accept rides.');
  }
...
export const getAdminBookings = asyncHandler(async (req, res) => {
  const result = await listAdminBookingsService(req.query, req.staff);
...
export const downloadDriverBookingInvoice = asyncHandler(async (req, res) => {
  const booking = await getBookingByIdService(req.params.id, { driverId: req.driver._id });
  if (!booking) {
    throw new ApiError(404, 'Booking not found');
  }
...
export const getEmergencyPoolBookings = asyncHandler(async (req, res) => {
  console.log('getEmergencyPoolBookings hit!');

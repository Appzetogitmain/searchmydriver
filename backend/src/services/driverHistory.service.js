...
export const getDriverPaymentHistoryService = async (driverId) => {
  const [payments, kitOrders, withdrawals] = await Promise.all([
    Payment.find({ driverId }).sort({ createdAt: -1 }).lean(),
    KitOrder.find({ driverId }).sort({ createdAt: -1 }).lean(),
    WithdrawalRequest.find({ driverId }).sort({ createdAt: -1 }).lean(),
  ]);
  ...
  for (const w of withdrawals) {
    items.push({
      ...
      orderNumber: w.transactionRef || '',
      amount: w.amount,
      currency: 'INR',
      status: w.status === 'pending' ? 'pending' : (w.status === 'approved' ? 'paid' : 'failed'),

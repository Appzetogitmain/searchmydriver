...
export async function rateDriverService(userId, bookingId, body = {}) {
  ...
  booking.rating.customer = {
    ...
    review,
    ratedAt: new Date(),
    questionResponses: Array.isArray(body?.questionResponses) ? body.questionResponses : [],
  };
  await booking.save();
...
export async function rateCustomerService(driverId, bookingId, body = {}) {
  ...
  try {
    ...
    if (updatedUser && updatedUser.ratingCount > 0) {
      const avg = round2(updatedUser.totalRatingScore / updatedUser.ratingCount);
      const clamped = Math.max(0, Math.min(5, avg));
    ...
    console.error(
      '[bookingRating] failed to update user rating aggregate:',
      err?.message
    );
  }

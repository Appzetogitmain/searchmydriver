...
export async function getWalletService(userId, userType = 'User') {
  if (!userId) throw new ApiError(400, 'userId is required');
  const Model = userType === 'Driver' ? Driver : User;
  const user = await Model.findById(userId).select('wallet name').lean();
...
export async function creditWalletService({
  ...
  if (!updated) throw new ApiError(404, `${userType} not found`);
    ...
    updated = await Model.findOneAndUpdate(
      {
        ...
        $expr: {
          $gte: [
            { $subtract: [{ $ifNull: ['$wallet.balance', 0] }, { $ifNull: ['$wallet.heldRupees', 0] }] },
            amt,
          ],
        },
      },
 ...
 */
export async function holdWalletService({ userId, amount, userType = 'User' }) {
  const amt = ensurePositive(amount, 'amount');
  const Model = userType === 'Driver' ? Driver : User;
  const updated = await Model.findOneAndUpdate(
    {
  ...
  if (!updated) {
    const wallet = await getWalletService(userId, userType).catch(() => null);
    const balance = wallet?.balance || 0;
    const heldRupees = wallet?.heldRupees || 0;
    const availableRupees = wallet?.availableRupees || 0;
 ...
 */
export async function releaseWalletHoldService({ userId, amount, userType = 'User' }) {
  const amt = ensurePositive(amount, 'amount');
  const Model = userType === 'Driver' ? Driver : User;
 ...
 */
export async function createTopupOrderService(userId, amount, userType = 'User') {
  const amt = ensurePositive(amount, 'amount');
  ...
  const Model = userType === 'Driver' ? Driver : User;
  const user = await Model.findById(userId).select('name email phone_no').lean();
  if (!user) throw new ApiError(404, `${userType} not found`);
 ...
 */
export async function verifyTopupPaymentService(userId, { orderId, paymentId, signature, userType = 'User' }) {
  ...
  if (pending.status === WALLET_TXN_STATUS.SUCCESS) {
    const wallet = await getWalletService(userId, userType);

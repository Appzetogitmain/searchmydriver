...
export async function verifyBookingPaymentService(userId, bookingId, { orderId, paymentId, signature }) {
  ...
  // Record platform revenue for Monthly rides immediately after payment is verified.
  if (booking.serviceType === 'monthly' && remaining <= 0 && booking.paymentStatus === BOOKING_PAYMENT_STATUS.PAID) {
    // Fire-and-forget: we don't want a ledger hiccup to wedge the payment callback
    recordPlatformRevenue({
      ...
      console.warn('[bookingPayment] Failed to record Monthly revenue:', err.message);
    });
  }

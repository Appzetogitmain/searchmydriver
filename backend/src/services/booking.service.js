...
import {
  cancelSearchingSchedule,
  resumeSearchingScheduleIfNeeded,
  scheduleSearchingTimeout,
} from './bookingSearchingTimeout.service.js';
import {
...
export function sanitizeBookingForDriver(booking) {
  ...
  if (obj.fareSnapshot) {
    ...
    obj.fareSnapshot = {
      ...
      offlineTip: Number(obj.offlineTip) || 0,
      currency: 'INR',
    };
  }
...
export async function getActiveBookingForUserService(userId) {
  ...
  resumeSearchingScheduleIfNeeded(booking).catch(() => {});
...
export async function createBookingService(userId, body) {
  ...
  const { serviceType, bookingType, carId, pickup, dropoff, hourly, outstation, monthly, offlineTip } = body;
...
export async function cancelBookingByUserService(userId, bookingId, reason = '') {
  ...
  try {
    ...
    // Notify admin on every 3rd cancellation
    if (cancelCount > 0 && cancelCount % 3 === 0) {
      const user = await User.findById(booking.userId).select('name email phone_no');
      if (user) {
        const alertPayload = {
          recipientModel: 'Admin',
          title: 'High User Cancellations Alert',
          body: `User ${user.name} (${user.phone_no || user.email}) has cancelled ${cancelCount} rides. Please review their account for possible suspension.`,
          severity: 'warning',
          data: { userId: booking.userId, cancelCount },
        };

        const notification = await Notification.create(alertPayload);
        emitToAdmins('NOTIFICATION_NEW', notification.toObject());
      }
    }
  } catch (err) {
...
export async function listAdminBookingsService(query = {}, staff = null) {
  const {
    page = 1,
    limit = 20,
    search,

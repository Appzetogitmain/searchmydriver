...
export async function settleDriverEarning(booking) {
  ...
  const totalPayable = round2(Number(breakdown.totalPayable) || 0);

  // If the user paid in cash, the driver collected the full `totalPayable` 
  // (which includes GST, service charges, and subtotal) from the customer.
  // The driver is only entitled to `driverEarning`.
  // Therefore, the driver owes the platform: (totalPayable - driverEarning)
  // We deduct this amount from the driver's wallet.

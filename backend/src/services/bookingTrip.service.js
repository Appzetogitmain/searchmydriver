...
export async function markDriverEnRouteService(driverId, bookingId) {
  ...
  updateDriverTripStatusLive(driverId, true).catch(() => {});
...
export async function startTripService(driverId, bookingId, { otp } = {}) {
  ...
  // Check if driver has a kit, if not send a warning
  try {
    ...
    console.warn('[bookingTrip] failed to check kit status on start:', err?.message);
  }
...
export async function completeTripService(driverId, bookingId) {
  ...
  if (booking.driverId) {
    ...
    updateDriverTripStatusLive(booking.driverId, false).catch(() => {});
  }
  ...
  // --- No Kit Penalty Logic ---
  let deductedPenalty = 0;
  try {
    const { getDriverKitEligibility } = await import('../utils/kitEligibility.util.js');
    const PlatformSettings = (await import('../models/platformSettings.model.js')).default;
    
    const eligibility = await getDriverKitEligibility(booking.driverId);
    if (!eligibility.hasKit) {
      ...
      if (penaltyAmount > 0) {
        ...
        recordPlatformRevenue({
          source: PLATFORM_REVENUE_SOURCE.NO_KIT_PENALTY,
          amountRupees: penaltyAmount,
          bookingId: booking._id,
          bookingNumber: booking.bookingNumber || '',
          serviceType: booking.serviceType || '',
          userId: null,
          driverId: booking.driverId || null,
          meta: { reason: 'Completed trip without a kit' },
        }).catch(() => {});

        deductedPenalty = penaltyAmount;
      }
    }
  } catch (err) {
    console.warn('[bookingTrip] failed to process no kit penalty:', err?.message);
  }
  // ----------------------------

  // Notify admins instantly that the trip completed and show commission collected
  emitNotification(
    { admin: true },
    {
      title: commission > 0 ? 'Commission Auto-Collected' : 'Trip Completed',
      body: commission > 0 
        ? `Trip ${booking.bookingNumber || ''} completed. ₹${commission} commission auto-deducted from driver's wallet.`
        : `Trip ${booking.bookingNumber || ''} completed. No commission was collected on this trip.`,
  ...
  if (booking.driverId) {
    ...
    const dEarn = round2(Number(bd.driverEarning) || 0);
    const tPay = round2(Number(bd.totalPayable) || 0);
    const isCash = booking.paymentMethod === 'cash';
    const amountOwed = round2(tPay - dEarn);
    const delta = isCash ? -amountOwed : dEarn;

    let dBody = `Your trip ${booking.bookingNumber || ''} has been completed successfully.`;
    if (delta < 0) {
      dBody += ` ₹${Math.abs(delta)} was deducted from your wallet for platform fees/taxes.`;
    } else if (delta > 0) {
      dBody += ` ₹${delta} has been credited to your wallet.`;
    }
    
    if (deductedPenalty > 0) {
      dBody += ` Additionally, ₹${deductedPenalty} was deducted because you have not purchased a Driver Kit.`;
    }
    ...
    emitNotification(
      { user: booking.driverId, userModel: 'Driver' },
      {
        ...
        body: dBody,
        severity: deductedPenalty > 0 ? 'warning' : 'success',
      }
    ).catch(() => {});
  }
...
async function redispatchAfterDriverCancel(booking, driverId, policy, chance) {
  ...
  updateDriverTripStatusLive(driverId, false).catch(() => {});

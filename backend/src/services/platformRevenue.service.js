...
export async function recordPlatformRevenue({
  ...
  if (amt === 0) return null; // allow negative for withdrawals
  if (!bookingId && !['wallet_top_up', 'wallet_withdrawal', 'manual_add', 'manual_deduct'].includes(source)) {
    throw new ApiError(400, 'bookingId is required for this source');
  }

  const doc = await PlatformRevenue.create({
    source,
    amountRupees: amt,
  ...
  // Increment the admin wallet balance in the background
  PlatformSettings.findOneAndUpdate(
    {},
    { $inc: { adminWalletBalance: amt } },
    { upsert: true }
  ).catch(err => console.warn('[PlatformRevenue] failed to increment adminWalletBalance:', err));

...
export const verifyUserOtpAndRegisterService = async ({ name, phone, password, otp, referralCode, city }) => {
  if (!name || !phone || !password || !otp || !city) {
...
export const updateUserProfileService = async (userId, data) => {
  ...
  if (data.profilePicture !== undefined) allowedUpdates.profilePicture = data.profilePicture;
  if (data.gender !== undefined) allowedUpdates.gender = data.gender;
  if (data.dateOfBirth !== undefined) allowedUpdates.dateOfBirth = data.dateOfBirth;
